    ...
    for layer in model.layers[1:]:
        ...
        for node in nodes:
            ...
            if type(inputs[0]) is tuple:
                if len(inputs[0]) == 2:
                    shape[layer.name] = layer.compute_output_shape(inputs[0][0])
                else:
                    # print(inputs[0] if (len(inputs) == 1) else inputs)
                    if layer.name == 'block1_dim_reduction' or layer.name == 'block2_dim_reduction':
                        shape[layer.name] = (None, None, None, None, 1)  # inputs[0] if (len(inputs) == 1) else inputs, 3
                    elif layer.name == 'lambda_1' or layer.name == 'splid2d':
                        shape[layer.name] = (None, None, None, 6)
                    elif layer.name == 'split':
                        shape[layer.name] = (None, None, None, 3, 2)
                    elif 'squeeze' in layer.name:
                        shape[layer.name] = (None, None, None, 3)
                    else:
...
def anchors_for_shape(
    ...
    if strides is None:
        # strides = [2 ** (x-1) for x in pyramid_levels]
        strides = [2 ** x for x in pyramid_levels]
    if sizes is None:
        # sizes = [2 ** (x + 2 - 1) for x in pyramid_levels]
        sizes = [2 ** (x + 2) for x in pyramid_levels]
    if ratios is None:
        ratios = np.array([0.5, 1, 2])
    if scales is None:
        #hughes
        scales = np.array([2 ** 0, 1 / 2 ** (1.0 / 3.0), 1 / 2 ** (2.0 / 3.0)])
...
def shift(shape, stride, anchors):
    ...
    # shift_x = (np.arange(0, shape[1]) + 0.5) * stride*2
    # shift_y = (np.arange(0, shape[0]) + 0.5) * stride*2
